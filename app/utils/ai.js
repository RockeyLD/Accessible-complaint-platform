...
**重要规则：推荐输入中不能包含任何具体地名、路名、站点名、区名等城市地理位置信息。使用通用描述代替。**
 ...
 * @returns {boolean} 是否包含地名
 */
function containsLocation(text) {
  // 常见地名关键词（城市、区县、街道、站点等）
  const locationPatterns = [
    ...
    // 常见城市
    '广州', '深圳', '杭州', '南京', '成都', '武汉', '西安', '郑州', '长沙', '沈阳',
    '青岛', '大连', '宁波', '厦门', '苏州', '无锡', '佛山', '东莞', '石家庄', '济南',
    // 地区/街道关键词
    '区', '县', '镇', '乡', '村', '街道', '大道', '路', '街', '巷', '胡同',
    // 交通站点
    '站', '机场', '港口', '码头',
    // 著名地点
    '广场', '大厦', '中心', '大楼', '商城', '商场', '超市',
    // 常见路名后缀
    '东路', '南路', '西路', '北路', '大街', '大道', '中路',
    // 数字+线路/站点
    /\d+号线/, /\d+路/, /[一二三四五六七八九十]+号线/,
    // 商圈/地标
    '天河', '浦东', '朝阳', '海淀', '罗湖', '福田', '南山', '罗湖', '西湖', '鼓楼',
    '玄武', '秦淮', '建邺', '下关', '秦淮', '玄武', '白下', '鼓楼', '玄武',
    // 医院名常见词
    '协和', '同仁', '华山', '瑞金', '中山', '华西', '湘雅', '齐鲁', '协和'
  ];
  ...
  if (suggestionMatch) {
    ...
    if (lines) {
      lines.forEach(line => {
        ...
        // 过滤掉包含地名的选项
        if (text && text.length > 0 && !containsLocation(text)) {
